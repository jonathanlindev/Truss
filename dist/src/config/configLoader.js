...
function loadTrussConfig(configPath) {
    ...
    try {
        ...
        throw new ConfigError(`Invalid YAML in ${configPath}: ${e.message}`);
    }
    ...
    for (const r of cfg.rules) {
        if (!r || typeof r !== "object")
            throw new ConfigError("Invalid rule entry: expected object");
        if (!r.name || typeof r.name !== "string")
            throw new ConfigError('Rule is missing "name"');
