export function evaluateRules(opts: {
  ...
  // #region agent log
  fetch("http://127.0.0.1:7861/ingest/8b9c63fd-394c-4722-bece-a02463c6f64a", { method: "POST", headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "8d2d4f" }, body: JSON.stringify({ sessionId: "8d2d4f", runId: "pre-fix", hypothesisId: "H2", location: "src/core/validator.ts:evaluateRules:entry", message: "evaluateRules entry", data: { edgeCount: edges.length, ruleCount: config.rules.length, layerCount: Object.keys(config.layers ?? {}).length }, timestamp: Date.now() }) }).catch(() => {});
  // #endregion
  ...
  const getLayer = (file: string): string | null => {
    ...
    if (!didLogMatchLayerProbe) {
      ...
      // #region agent log
      fetch("http://127.0.0.1:7861/ingest/8b9c63fd-394c-4722-bece-a02463c6f64a", { method: "POST", headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "8d2d4f" }, body: JSON.stringify({ sessionId: "8d2d4f", runId: "pre-fix", hypothesisId: "H1", location: "src/core/validator.ts:getLayer:beforeMatchLayer", message: "matchLayer probe before invocation", data: { probeFile: file, matchLayerType: typeof matchLayer }, timestamp: Date.now() }) }).catch(() => {});
      // #endregion
    }
